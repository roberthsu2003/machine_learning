...
### 1. 手動公式實作：循序漸進理解底層 ($y = Xw + b$)
> 適合想掌握演算法底層邏輯、為深度學習奠定扎實數學基礎的學習者。
> * **💡 優點**：能完全掌握權重 ($w$) 與偏差 ($b$) 的更新細節與數學原理。
> * **⚠️ 缺點**：程式實作較為繁瑣，需要自行處理損失函數與梯度下降的計算。

- [**簡單線性迴歸_手動實作步驟**](./簡單線性迴歸_手動.md)
- [📝 Cost Function (損失函數) 實作](./cost_function.ipynb)
- [📉 Gradient Descent (梯度下降) 實作](./gradient_descent.ipynb)

---

### 2. 套件應用實作：直接使用 scikit-learn
> 適合想快速上手、著重於機器學習套件應用與模型評估的學習者。
> * **💡 優點**：程式碼簡潔，僅需數行即可快速完成模型建構與預測。
> * **⚠️ 缺點**：底層數學被封裝，初學時較不易直觀感受 $w$ 與 $b$ 的求解過程。
