...
> 💡 **直覺理解「批次大小」的影響**：
> - **批次大小大（如大型民調）**：每次參考很多資料，能穩定反映真實分布。因此模型每次更新方向都很精確（訓練穩定），但一次要加載這麼多資料，GPU/RAM 的記憶體開銷就會非常高。
> - **批次大小小（如街頭問 5 個人）**：每次只讀幾筆資料，記憶體消耗極低。但如果這幾筆資料剛好有極端值，模型更新方向就會忽左忽右、大幅震盪（訓練不穩定）。不過，這種「搖晃」有時能幫模型跳出局部盲點（Local Minima），找到更好的全局最佳解！
