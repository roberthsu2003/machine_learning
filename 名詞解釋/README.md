...
### 2. 數據結構
- [2.1 特徵（Features）](#1-特徵features)
- [2.2 標籤（Labels）](#2-標籤labels)
- [2.3 簡單範例](#簡單範例)

### 3. 數據分割
- [3.1 訓練集（Training Set）](#1-訓練集training-set)
- [3.2 測試集（Test Set）](#2-測試集test-set)
- [3.3 分割的目的](#分割的目的)
- [3.4 簡單範例](#簡單範例-1)

### 4. 特徵工程
- [4.1 特徵選擇](#特徵選擇feature-selection)
- [4.2 特徵縮放](#特徵縮放feature-scaling)
- [4.3 編碼方式](#編碼方式)

### 5. 模型參數
- [5.1 參數（Parameters）](#1-參數parameters)
- [5.2 超參數（Hyperparameters）](#2-超參數hyperparameters)
- [5.3 參數與超參數的區別](#3-參數與超參數的區別)

### 6. 優化算法
- [6.1 梯度下降法](#梯度下降法gradient-descent)
- [6.2 學習率](#學習率learning-rate)
- [6.3 批次大小](#批次大小batch-size)

### 7. 常見算法
- [7.1 K-近鄰](#k-近鄰k-nn)
- [7.2 決策樹](#決策樹decision-tree)
- [7.3 支援向量機](#支援向量機svm)
- [7.4 樸素貝氏](#樸素貝氏naive-bayes)

### 8. 集成學習
- [8.1 Bagging](#baggingbootstrap-aggregating)
- [8.2 Boosting](#boosting)
- [8.3 Stacking](#stacking)

### 9. 評估指標
- [9.1 分類指標](#分類指標)
- [9.2 回歸指標](#回歸指標)
- [9.3 混淆矩陣](#混淆矩陣confusion-matrix)

### 10. 模型性能問題
- [10.1 欠擬合（Underfitting）](#1-欠擬合underfitting)
- [10.2 過度擬合（Overfitting）](#2-過度擬合overfitting)
- [10.3 總結與解決方案](#總結與解決方案)

### 11. 模型泛化
- [11.1 模型的泛化（Generalization）](#模型的泛化generalization)
- [11.2 對比欠擬合與過度擬合](#對比欠擬合與過度擬合)
- [11.3 實現良好泛化的方法](#實現良好泛化的方法)

### 12. 機器學習訓練過程
- [12.1 ML的整個「訓練過程」](#ml的整個訓練過程這裡以監督式學習supervised-learning為例)
- [12.2 泛化能力說明](#泛化能力說明)
- [12.3 欠擬合與過度擬合比較表](#欠擬合與過度擬合比較表)
- [12.4 圖解說明](#圖解說明)
...
### 特徵選擇 (Feature Selection) {#特徵選擇feature-selection}
**描述**：  
...
### 特徵縮放 (Feature Scaling) {#特徵縮放feature-scaling}
**描述**：  
...
### 學習率 (Learning Rate) {#學習率learning-rate}
**描述**：  
控制參數更新步長的超參數。
...
### 批次大小 (Batch Size) {#批次大小batch-size}
**描述**：  
每次迭代中使用的訓練樣本數量。
...
### 決策樹 (Decision Tree) {#決策樹decision-tree}
**描述**：  
基於樹狀結構的分類和回歸算法。
...
### 樸素貝氏 (Naive Bayes) {#樸素貝氏naive-bayes}
**描述**：  
...
#### 混淆矩陣 (Confusion Matrix) {#混淆矩陣confusion-matrix}
**描述**：  
