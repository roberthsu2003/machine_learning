# [機器學習書本說明和範例(適合教學說明)](../多元線性迴歸/書本說明和範例.ipynb)
本範例取材自經典機器學習教材，詳細說明線性模型（Linear Models）在迴歸任務中的基本原理與實作。內容涵蓋普通最小平方法 (OLS)、嶺迴歸 (Ridge Regression) 以及 Lasso 迴歸，透過波士頓房價資料集深入探討模型複雜度控制、L1/L2 正則化、特徵選擇與學習曲線 (Learning Curves) 分析。
