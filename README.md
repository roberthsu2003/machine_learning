...
本專案針對不同的課程時數規劃了推薦的學習路徑。不論是短期的 **12 小時速成班** 還是長期的 **36+ 小時完整專案班**，皆可靈活套用本專案的教學模組：
...
1. **基礎準備 (9小時)**：[何謂 AI](./簡報圖片) ➡️ [常用術語](./名詞解釋) ➡️ [NumPy 基礎](./使用數據) ➡️ [資料視覺化](./機器學習常使用的圖表) ➡️ [機率統計](./機率統計) ➡️ [Jupyter 實作](./基本package/README.ipynb)
2. **資料管線 (6小時)**：[特徵工程](./特徵工程) ➡️ [模型評估指標](./評估指標)
3. **經典演算法 (15小時)**：[k-NN 分類](./k近鄰分類) ➡️ [線性迴歸](./簡單線性迴歸)/[多元迴歸](./多元線性迴歸) ➡️ [邏輯迴歸](./邏輯迴歸) ➡️ [貝氏分類](./貝氏分類) ➡️ [決策樹](./樹狀模型) ➡️ [隨機森林與集成](./決策樹集成模型) ➡️ [SVM](./核化支援向量機) ➡️ [薈萃學習 Stacking](./薈萃式學習)
4. **進階領域 (6小時)**：[集群分析](./集群分析) ➡️ [深度學習與神經網路](./深度學習)
